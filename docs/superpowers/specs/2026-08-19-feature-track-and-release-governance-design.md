...
当功能已按用户确认通过人工集成流程进入 `main`，不得伪造 `ready_to_merge` 状态或复用过期候选清单。该命令先以 `--preview` 只读输出绑定当前 `main` 提交、版本和已集成功能列表的摘要；只有精确提供该摘要且全量门禁通过后，才原子写入候选清单和集成回执。验收期间 HEAD 变化、工作区变脏、非 `main` 或门禁失败均不产生有效回执。后续仍必须使用同一摘要进行独立的 `release-build` 发布确认。
