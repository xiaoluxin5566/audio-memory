...
同一文件中 `segment_id` 必须唯一；重复 ID 会以同一通用错误拒绝，
防止重复一条标注虚增“稳定样本数”。

## 可选模型信号的持久化门槛

生产快速转写仅在 MLX 对未拆分的句段返回有限数时，持久化
`no_speech_prob` 和 `avg_logprob`；缺失、非有限数、概率越界或说话人
拆分后无法无歧义归属的值保存为 `NULL`。该信号当前只作脱敏校准
候选：未进入本评测器的 v1 输入模式，也不会修改中/高风险结果。
只有在去标识化导出、唯一匿名 ID、每类至少 10 个人工标注样本且
ROC 审批完成后，才能扩展评测模式或启用阈值。
