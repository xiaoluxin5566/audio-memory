...
Prompt 评测默认使用离线已保存样例，不读取 Keychain，不调用 Kimi、DeepSeek 或 OpenAI。脚本同时提供显式的 `--provider kimi|deepseek|openai` 能力：它只能从 macOS Keychain 取得凭据，并把脱敏聚合报告以 `0600` 权限写入私有的 `0700` 运行目录。报告不包含转写、原始生成内容、样例路径或密钥。真实厂商模式仍非默认发布门禁，本次验证只使用假后端，未读取 Keychain 或发起网络请求。
