...
- A compatible EventMap is seeded only when the current source version persists and matches the exact system/event-map/common Prompt hashes, schema-version hash, actual EventMap/six-scene JSON-Schema hash, frozen-profile canonical hash, and versioned structured-transcript fingerprint. The source version's actual `profile_snapshot_json` is parsed and canonical-hashed again; both its content and persisted compatibility metadata must match the new batch profile hash. The transcript fingerprint covers ordered file identity/position/recording metadata/speech mapping and every segment's UID, speaker, timing, text, and word JSON. The EventMap must also parse against the current strict Schema, match its stored content hash, and have assigned plus unassigned evidence IDs exactly equal the current structured-transcript segment IDs. A profile mismatch merely regenerates EventMap; it does not pause. Legacy or inconsistent metadata also safely regenerates.
...
- A profile checkpoint is background-actionable only while `completed_at IS NULL`. Once the sole automatic attempt fails, its non-null completion marker keeps the old batch out of Worker selection, so it cannot poll or starve newer pending/running/stopping batches; explicit profile retry remains available.
...
23. Profile-failure starvation: an old `content_completed_profile_failed` batch with a completed automatic attempt was selected forever and prevented a newer pending batch from enqueueing. Worker selection now requires `completed_at IS NULL` for that state; explicit retry with a non-null marker remains green.
24. Mixed resume reasons: an earlier ordinary item error hid a later Schema/transcript blocker and resume cleared both. Resume now queries for existence of any blocking error across all batch items and deterministically rejects without mutation.
25. EventMap profile defense: matching metadata could hide a different `AnalysisVersion.profile_snapshot_json`. Compatibility now parses the actual JSON, canonical-hashes it, and requires equality with the persisted profile hash; malformed or inconsistent content regenerates EventMap.

Each RED was followed by a focused GREEN before the next behavior was implemented.

## Verification results

- Required Task 8 suite (`tests/unit/reanalysis`, API, Worker): 37 passed in 3.77s.
- Focused fix-round selection (reanalysis, publisher, EventMap, coordinator, migrations): 90 passed in 5.94s.
- Full backend suite: 442 passed in 10.51s.
