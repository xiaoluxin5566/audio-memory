...
test('active transcription disables adding audio until its report is published', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: '移除 current.mp3' })).toBeEnabled()
  await expect(page.getByText('上传完成')).toBeVisible()
  await page.getByRole('button', { name: '移除 current.mp3' }).click()
  await expect(page.getByRole('dialog')).toContainText('任务进行中，不能删除音频文件')
})
