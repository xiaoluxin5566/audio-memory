...
function JobPanel({ job, onRetry, onCancel }) {
  ...
  if (job.stage === 'failed') return <div className="job-card error"><b>模型分析失败</b><p>已保留完整转写；可修改当前厂商后重新分析，不会再次执行 Whisper。</p><div><button className="secondary" onClick={onCancel}>放弃任务</button><button className="primary" onClick={onRetry}>重新分析</button></div></div>;
...
function TodoRow({ todo, refresh, editingTodo, setEditingTodo }) {
  ...
  return <div className={`todo-row ${todo.completed ? 'completed' : ''} ${todo.overdue ? 'overdue' : ''}`}><button className={`todo-check ${todo.completed ? 'checked' : ''}`} onClick={async () => { await api.updateTodo(todo.id, { completed: !todo.completed }); await refresh(); }}>{todo.completed ? '✓' : ''}</button><div className="todo-copy">{editing ? <input value={draft} onChange={(event) => setDraft(event.target.value)} autoFocus /> : <b>{todo.text}</b>}<small>{todo.due}</small></div><div className="todo-actions">{editing ? <button onClick={async () => { if (draft.trim()) await api.updateTodo(todo.id, { text: draft.trim() }); setEditingTodo(null); await refresh(); }}>保存</button> : <button onClick={() => { setDraft(todo.text); setEditingTodo(todo.id); }}>编辑</button>}<button className="delete-link" onClick={async () => { await api.deleteTodo(todo.id); await refresh(); }}>删除</button></div></div>;
}
