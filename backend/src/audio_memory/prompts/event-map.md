任务：将当前一个有界局部窗口的结构化转写整理为客观、可复用的事件地图。只识别事件和还原事实，不生成建议，不评价用户表现。服务端会把多个局部窗口安全合并，因此必须优先保证当前窗口内的事件召回和证据边界，不要尝试概括整次上传。
...
即使输入属于同一局部窗口，招聘面谈或电话、职业讨论、产品与业务讨论、负责人沟通、媒体播放、亲子互动、明确承诺以及活动目标明显改变时也必须拆成独立事件。只要存在这些核心证据，就不得因为语气随意或场景非正式而把整个窗口统一标为 casual_chat。连续闲聊只有在确实没有工作、面试、媒体、亲子、承诺或深度讨论价值时才能使用 casual_chat。
