"""Prepare and run the approved Beta 8 P1-P5 first-result pipeline."""
...
def main(argv=None):
    ...
    try:
        ...
        binding = {"input_sha256": digest(segments), "prompt_hash": P1P5Prompts.fixed_rules_hash()}
        if args.command == "prepare":
            limits = WritingLimits()
            windows = P1P5EvidenceCatalog(segments).windows(limits.core_budget_bytes)
            capacities = [{"window_id": window["window_id"], "core_segment_count": len(window["core_segments"]),
                           "input_byte_upper_bound": len(json.dumps({"model": "deepseek-v4-pro", "messages": [
                               {"role": "system", "content": P1P5Prompts.system("P1")},
                               {"role": "user", "content": P1P5Prompts.user(dict(window, user_corrections=[]))}],
            ...
            plan = dict(binding, pipeline="beta8_p1_p5_v1", provider_id="deepseek", model_id="deepseek-v4-pro",
                ...
                counts={"P1_windows": len(windows), "P2_planning": 1 if windows else 0, "P3_requests": None, "P4_cards": None, "P5_scoring": None, "P5_scoring_note": "N：每张主卡 1 次", "total_formula": "W + 1 + H + N + N"},
        ...
        if any(grant.get(key) != value for key, value in binding.items()) or grant.get("pipeline") != "beta8_p1_p5_v1":
        ...
        async def execute():
            ...
            async with httpx.AsyncClient(trust_env=False) as client:
                ...
                pipeline = P1P5Pipeline(output_dir=root / "artifacts", transport=transport, limits=limits,
        ...
        return 0 if result["status"] in {"scored_v1", "draft_ready_scoring_incomplete", "no_cards_planned"} else 2
