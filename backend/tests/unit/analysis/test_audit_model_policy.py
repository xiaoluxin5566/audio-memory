...
def test_known_models_receive_distinct_operational_audit_budgets() -> None:
    ...
    assert glm.policy_name == "glm-5.2-output-bounded-v2"
    assert glm.max_transcript_tokens == 8_000
...
def test_unknown_model_uses_the_most_conservative_supported_policy() -> None:
    ...
    assert glm.max_transcript_tokens < unknown.max_transcript_tokens
